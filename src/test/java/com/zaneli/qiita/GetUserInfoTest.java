...
public class GetUserInfoTest {
  ...
  @Test
  public void 特定ユーザーの情報取得成功(@Mocked("execute") QiitaExecutor executor) throws Exception {
    QiitaClientTestUtil.configureMock(executor, SC_OK, "UserInfo.json");
    QiitaClient client = new QiitaClient();
    UserInfo info = client.getUserInfo("yaotti");
    assertEquals("Hiroshige Umino", info.getName());
    assertEquals("yaotti", info.getUrlName());
    assertEquals("https://si0.twimg.com/profile_images/2309761038/1ijg13pfs0dg84sk2y0h_normal.jpeg", info.getProfileImageUrl());
  ...
  @Test
  public void 特定ユーザーの情報取得失敗(@Mocked("execute") QiitaExecutor executor) throws Exception {
    QiitaClientTestUtil.configureMock(executor, SC_BAD_REQUEST, "Error.json");
    QiitaClient client = new QiitaClient();
    try {
      client.getUserInfo("yaotti");
